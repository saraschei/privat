...
class ColumnCase3():
    ...
    def iteration(self):
        ...
        if cy2 < deltahHorz:
            ...
            Asy2Start = int(cy2*np.sin(alfa1)/deltah)
        ...
        if Asy2Finish==n:
            ...
            #Asy1
        Asy1Start = int((b-cy1)/deltahHorz )
    ...
    def plot(self, loop, epsciMat, sigciMat, sigsjMat ): 
        ...
        #R = np.array([[self.R[0][0] * np.cos (self.__alfa1) * self.__b/1000], [self.R[1][0] * np.cos (self.__alfa1) * self.__b/1e6], [self.R[1][0] * np.sin (self.__alfa1) * self.__h/1e6]])
