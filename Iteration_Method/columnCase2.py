...
class ColumnCase2():
    ...
    def iteration(self):
        ...
        if cy1 < deltah/np.cos(alfa2):
            ...
            Asy1Start = int(cy1*np.cos(alfa2)/deltah)
            
        Asy1Finish = Asy1Start + int(h * np.sin(alfa2)/deltah)
        
        
            #Asy2
        Asy2Start = int((b-cy2)*np.cos(alfa2)/deltah)
        ...
        if RN !=0:
            RNAbs = abs(RN)
            betaN = RNAbs * beta  #doesn't have any effect 
            
            if RM != 0:
                RMAbs = abs(RM)
                betaM = RMAbs/1000 * beta   #doesn't have any effect
                
            else:       #RM==zeros 
                betaM = betaN       #does have effect
                
        else:           #RN == zeros
            RMAbs = abs(RM)
            betaM = RMAbs/1000 * beta       #doesn't have any effect 
            betaN = betaM                   #does have effect
        ...
        while devMax > beta or diffNMax > betaN or diffMMax > betaM:
            ...
            self.mxSList.append(S[1][0] * np.sin(alfa2) * b/1e6)    #converted to kNm
            self.mySList.append(S[1][0] * np.cos(alfa2) * h/1e6)    #converted to kNm
        ...
        SOrig[1][0] = S[1][0] * np.sin(alfa2)* b/1e6
        SOrig[2][0] = S[1][0] * np.cos(alfa2)* h/1e6
