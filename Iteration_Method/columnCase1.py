...
class ColumnCase1():
    ...
    def __init__(self, nx, mx, my, fck, fyk, gammac, gammas, concModel, Asx1, Asx2, Asy1, Asy2,\
        ...
        self.__nx =  nx * 1000 * np.cos(abs(alfa1)) / b            #N/mm
        ...
        self.fyd = fyk/gammas
            ...
            #Geometry data
        self.hc = h/2 - b/2 * np.tan(self.__alfa1)
        self.hz = (h-self.hc) * np.cos(self.__alfa1)
        self.deltah = 2*self.hz/n
    ...
    def iteration(self):
        ...
        if alfa1==0:                                 #layer horizontal width
            deltahHorz = np.inf                        
        else: 
            deltahHorz = deltah/np.sin(alfa1)
            
        
        #Reinforcement positions
            #Asx1
        if cx1 < deltah/np.cos(alfa1):
            Asx1Start = 0
        else:
            Asx1Start = int(cx1/(deltah/np.cos(alfa1)))
        
        Asx1Finish = Asx1Start + int(b * np.sin(alfa1) / deltah)    
        
            #Asx2
        Asx2Start = int((h-cx2)/(deltah / np.cos(alfa1)))
        
        if cx2 > deltah/np.cos(alfa1):
            Asx2Finish = Asx2Start + int(b * np.sin(alfa1)/deltah)
        else:
            Asx2Finish = n-1
        ...
        if cy1 < deltahHorz:
            ...
            Asy1Start = int(cy1*np.sin(alfa1)/deltah)
            
        Asy1Finish = Asy1Start + int(h * np.cos(alfa1)/deltah)
        if Asy1Finish==n:
            Asy1Finish= n-1
        
            #Asy2
        Asy2Start = int((b-cy2)/deltahHorz )
        
        if cy2 > deltahHorz:
            Asy2Finish = Asy2Start + int(h * np.cos(alfa1) / deltah)
        else:
            Asy2Finish = n-1
        ...
        while devMax > beta or diffNMax > betaN or diffMMax > betaM:
            ...
            if start ==0:
                Kc = np.zeros((2,2))
                for i in range(n):
                    if abs(zc[i])< hc*np.cos(alfa1):
                        KcTemp = (b/np.cos(alfa1)) *  np.array([[Cc0, -zc[i]*Cc0], [-zc[i]*Cc0, ((zc[i])**2)*Cc0]])  #temporary Kc
                    else:
                        if alfa1==0:
                            KcTemp = ((hz-abs(zc[i]))*b) *  np.array([[Cc0, -zc[i]*Cc0], [-zc[i]*Cc0, ((zc[i])**2)*Cc0]])
                        else:
                            KcTemp = ((hz-abs(zc[i]))*(np.tan(alfa1)+1/np.tan(alfa1))) *  np.array([[Cc0, -zc[i]*Cc0], [-zc[i]*Cc0, ((zc[i])**2)*Cc0]])  #temporary Kc
                    Kc = Kc + KcTemp
                Kc = deltah * np.cos(alfa1)/b * Kc   
            ...
            for i in range(n):
                ...
                if abs(zc[i]) < hc*np.cos(alfa1):
                    Ai = deltah * b/np.cos(alfa1)
                else:
                    if alfa1==0:
                        Ai = deltah * ((hz-abs(zc[i]))*b) 
                    else:
                        Ai = deltah * ((hz-abs(zc[i]))*(np.tan(alfa1)+1/np.tan(alfa1)))
            ...
            self.nxSList.append(S[0][0] * b/(1000 * np.cos(alfa1)))   #converted to kN
            self.mxSList.append(S[1][0] * np.cos(alfa1) * b/1e6)    #converted to kNm
            self.mySList.append(S[1][0] * np.sin(alfa1) * h/1e6)    #converted to kNm
            ...
            for i in range(n):
                if abs(zc[i])< hc*np.cos(alfa1):
                    KcNewTemp =(b/np.cos(alfa1)) * np.array([[CcgiMat[0,i], -zc[i]*CcgiMat[0,i]], [-zc[i]*CcgiMat[0,i], ((zc[i])**2)*CcgiMat[0,i]]])  #temporary KcNew
                else:
                    if alfa1==0:
                        KcNewTemp = ((hz-abs(zc[i]))*b) *  np.array([[CcgiMat[0,i], -zc[i]*CcgiMat[0,i]], [-zc[i]*CcgiMat[0,i], ((zc[i])**2)*CcgiMat[0,i]]])
                    else:
                        KcNewTemp = ((hz-abs(zc[i]))*(np.tan(alfa1)+1/np.tan(alfa1))) *  np.array([[CcgiMat[0,i], -zc[i]*CcgiMat[0,i]], [-zc[i]*CcgiMat[0,i], ((zc[i])**2)*CcgiMat[0,i]]])
                KcNew = KcNew + KcNewTemp
                
            KcNew = deltah * np.cos(alfa1)/b * KcNew   
        ...
        SOrig = np.zeros((3,1))
        SOrig[0][0] = S[0][0] * b / (np.cos(alfa1) * 1000)
        SOrig[1][0] = S[1][0] * np.cos(alfa1)* b/1e6
        SOrig[2][0] = S[1][0] * np.sin(alfa1)* h/1e6
